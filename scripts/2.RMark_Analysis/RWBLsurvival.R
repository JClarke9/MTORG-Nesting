...
RWBL3.run <- function()
{
  ...
  S.bhcon = list(formula = ~1 + Time + I(Time^2) + Nestling + BHCONum)
...
RWBL4.run <- function()
{
  ...
  S.grazed = list(formula = ~1 + Time + I(Time^2) + Nestling + BHCONum + grazed)
  
  # 3. DSR varies with the previous Year + Nestlings grazing intensity
  S.pDoD = list(formula = ~1 + Time + I(Time^2) + Nestling + BHCONum + pDoD)
...
RWBL5.run <- function()
{
  ...
  S.kbg = list(formula = ~1 + Time + I(Time^2) + Nestling + BHCONum + KBG)
  
  # 3. DSR varies with Smooth Brome (correlated with KBG and Litter Depth)
  S.brome = list(formula = ~1 + Time + I(Time^2) + Nestling + BHCONum + SmoothB)
  
  # 4. DSR varies with Litter (correlated with KBG) - NA values
  # S.lit = list(formula = ~1 + Time + I(Time^2) + Nestling + BHCONum + Litter)
  
  # 5. DSR varies with Bare - NA values
  # S.bare = list(formula = ~1 + Time + I(Time^2) + Nestling + BHCONum + Bare)
  
  # 6. DSR varies with Forb
  S.forb = list(formula = ~1 + Time + I(Time^2) + Nestling + BHCONum + Forb)
  
  # 7. DSR varies with Grasslike  (correlated with KBG)
  S.grass = list(formula = ~1 + Time + I(Time^2) + Nestling + BHCONum + Grasslike)
  
  # 8. DSR varies with Woody
  S.woody = list(formula = ~1 + Time + I(Time^2) + Nestling + BHCONum + Woody)
  
  # 9. DSR varies with Litter Depth (correlated with VOR) - NA values
  # S.litdep = list(formula = ~1 + Time + I(Time^2) + Nestling + BHCONum + LitterD)
  
  # 10. DSR varies with Veg Height (correlated with VOR)
  S.height = list(formula = ~1 + Time + I(Time^2) + Nestling + BHCONum + Veg.Height)
  
  # 11. DSR varies with VOR
  S.vor = list(formula = ~1 + Time + I(Time^2) + Nestling + BHCONum + VOR)
...
(RWBL.real <- as.data.frame(RWBL5.results$S.bhcon$results$real) |> 
    ...
    dplyr::select(Stage, estimate, se, lcl, ucl) |> 
    group_by(Stage) |> 
...
RWBL.beta <- coef(RWBL5.results$S.bhcon) |>
  cbind(confint(RWBL5.results$S.bhcon, level = 0.85)) |> 
...
filter(RWBL.ddl, 
       S.Nestling == 0 & S.time %in% c(1:75) & S.Year == 2021| 
