...
coef(YHBL4.results$S.grazed)
confint(YHBL4.results$S.grazed, level = 0.85)

coef(YHBL4.results$S.pDoD)
confint(YHBL4.results$S.pDoD, level = 0.85)
...
YHBL.beta <- coef(YHBL5.results$S.grazing) |>
  cbind(confint(YHBL5.results$S.grazing, level = 0.85)) |> 
...
(YHBL.plot <- ggplot(YHBL.beta[4:6,], 
                     aes(x = Variable,
                         y = Coefficient)) +
...
inc1 <- which(pDoD.pred$estimates$par.index == 1)
inc18 <- which(pDoD.pred$estimates$par.index == 18)
inc36 <- which(pDoD.pred$estimates$par.index == 36)
inc54 <- which(pDoD.pred$estimates$par.index == 54)
inc72 <- which(pDoD.pred$estimates$par.index == 72)
nst1 <- which(pDoD.pred$estimates$par.index == 220)
nst18 <- which(pDoD.pred$estimates$par.index == 237)
nst36 <- which(pDoD.pred$estimates$par.index == 255)
nst54 <- which(pDoD.pred$estimates$par.index == 273)
nst72 <- which(pDoD.pred$estimates$par.index == 291)

pDoD.pred$estimates$Group <- NA
pDoD.pred$estimates$Group[inc1] <- "Incubating1"
pDoD.pred$estimates$Group[inc18] <- "Incubating18"
pDoD.pred$estimates$Group[inc36] <- "Incubating36"
pDoD.pred$estimates$Group[inc54] <- "Incubating54"
pDoD.pred$estimates$Group[inc72] <- "Incubating72"
pDoD.pred$estimates$Group[nst1] <- "Nestling1"
pDoD.pred$estimates$Group[nst18] <- "Nestling18"
pDoD.pred$estimates$Group[nst36] <- "Nestling36"
pDoD.pred$estimates$Group[nst54] <- "Nestling54"
pDoD.pred$estimates$Group[nst72] <- "Nestling72"

pDoD.pred$estimates$Stage <- NA
pDoD.pred$estimates$Stage[inc1] <- "Incubating"
pDoD.pred$estimates$Stage[inc18] <- "Incubating"
pDoD.pred$estimates$Stage[inc36] <- "Incubating"
pDoD.pred$estimates$Stage[inc54] <- "Incubating"
pDoD.pred$estimates$Stage[inc72] <- "Incubating"
pDoD.pred$estimates$Stage[nst1] <- "Nestling"
pDoD.pred$estimates$Stage[nst18] <- "Nestling"
pDoD.pred$estimates$Stage[nst36] <- "Nestling"
pDoD.pred$estimates$Stage[nst54] <- "Nestling"
pDoD.pred$estimates$Stage[nst72] <- "Nestling"

pDoD.pred$estimates$Day <- NA
pDoD.pred$estimates$Day[inc1] <- "1"
pDoD.pred$estimates$Day[inc18] <- "18"
pDoD.pred$estimates$Day[inc36] <- "36"
pDoD.pred$estimates$Day[inc54] <- "54"
pDoD.pred$estimates$Day[inc72] <- "72"
pDoD.pred$estimates$Day[nst1] <- "1"
pDoD.pred$estimates$Day[nst18] <- "18"
pDoD.pred$estimates$Day[nst36] <- "36"
pDoD.pred$estimates$Day[nst54] <- "54"
pDoD.pred$estimates$Day[nst72] <- "72"

(YHBLpDoD.plot <- ggplot(transform(pDoD.pred$estimates,
                                     Day = factor(Day, levels = c("1", "18", "36",
                                                                  "54", "72"))), 
                           aes(x = pDoD, 
                               y = estimate,
...
ggsave(YHBLpDoD.plot,
       filename = "outputs/figs/pDoDYHBL.png",
       dpi = "print",
       bg = "white",
       height = 6,
       width = 6)
