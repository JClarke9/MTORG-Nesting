...
nest <- nest |> 
  mutate(R1 = case_when(R1 == "18+" ~ "20",
                        R1 == "13+" ~ "NA",
                        .default = R1),
         R2 = case_when(R2 == "18+" ~ "20",
                        R2 == "13+" ~ "NA",
                        R2 == "15+" ~ "NA",
                        .default = R2),
         R3 = case_when(R3 == "18+" ~ "20",
                        R3 == "13+" ~ "NA",
                        .default = R3),
         R4 = case_when(R4 == "18+" ~ "20",
                        R4 == "13+" ~ "NA",
                        .default = R4))
...
for (i in unique(nest$Spec)) {
  ...
  spec.surv <- spec.surv |> 
    mutate(cTreat = case_when(cTreat == "Heavy" ~ "68",
                              cTreat == "Full" ~ "49",
                              cTreat == "Moderate" ~ "39",
                              cTreat == "Rest" ~ "0",
                              .default = NA),
           pTreat = case_when(pTreat == "Heavy" ~ "68",
                              pTreat == "Full" ~ "49",
                              pTreat == "Moderate" ~ "39",
                              pTreat == "Rest" ~ "0",
                              .default = NA))
...
spec.nest <- spec.nest |> 
  dplyr::select(Year, id, Spec, FirstFound, LastPresent, 
