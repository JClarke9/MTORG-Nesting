...
################################ Building Correlation Matrix Function ################################################
...
################################ Running Correlation Matix on Data ################################################


bird <- read_csv("working/RMarknesting.csv") |> 
  filter(Spec == "MODO" & Stage != "Laying")

a <- ggpairs(bird[20:29],                                        # create the matrix and put it in an object -- allows it to
