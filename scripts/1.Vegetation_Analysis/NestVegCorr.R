...
bird.surv <- filter(nest, Spec == "WEME")

# Creating a PCA ----------------------------------------------------------

veg <- select(bird.surv, KBG:VOR)

outlierKBG <- bird.surv |> 
  filter(KBG > quantile(bird.surv$KBG, 0.95, 
                               na.rm = T)) |> 
  select(id, Year, Paddock, Fate, KBG)

outlierBrome <- bird.surv |> 
  filter(SmoothB > quantile(bird.surv$SmoothB, 0.95, 
                               na.rm = T)) |> 
  select(id, Year, Paddock, Fate, SmoothB)

outlierLitter <- bird.surv |> 
  filter(Litter > quantile(bird.surv$Litter, 0.95, 
                               na.rm = T)) |> 
  select(id, Year, Paddock, Fate, Litter)

outlierBare <- bird.surv |> 
  filter(Bare > quantile(bird.surv$Bare, 0.95, 
                               na.rm = T)) |> 
  select(id, Year, Paddock, Fate, Bare)

outlierForb <- bird.surv |> 
  filter(Forb > quantile(bird.surv$Forb, 0.95, 
                               na.rm = T)) |> 
  select(id, Year, Paddock, Fate, Forb)

outlierGrass <- bird.surv |> 
  filter(Grasslike > quantile(bird.surv$Grasslike, 0.95, 
                              na.rm = T)) |> 
  select(id, Year, Paddock, Fate, Grasslike)

outlierWoody <- bird.surv |> 
  filter(Woody > quantile(bird.surv$Woody, 0.95, 
                               na.rm = T)) |> 
  select(id, Year, Paddock, Fate, Woody)

outlierLitterD <- bird.surv |> 
  filter(LitterD > quantile(bird.surv$LitterD, 0.95, 
                            na.rm = T)) |> 
  select(id, Year, Paddock, Fate, LitterD)

outlierHeight <- bird.surv |> 
  filter(Veg.Height > quantile(bird.surv$Veg.Height, 0.95, 
                               na.rm = T)) |> 
  select(id, Year, Paddock, Fate, Veg.Height)

outlierVOR <- bird.surv |> 
  filter(VOR > quantile(bird.surv$VOR, 0.95, 
                        na.rm = T)) |> 
